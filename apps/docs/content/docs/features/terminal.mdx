...
Coodeen includes a built-in terminal emulator powered by [xterm.js](https://xtermjs.org/). It connects to a pseudo-terminal (PTY) on your machine via Electron IPC, giving you a full shell experience without leaving the editor.

## Features

- **Full terminal emulation** — Runs your default shell (bash, zsh, etc.)
- **Native PTY** — Uses node-pty for real terminal behavior (colors, tab completion, etc.)
